...
!.. radar module.
 ...
 print *, "MP_WSM6_INIT SUCCESSFULLY CALLED"
 ! error stop "DEBUGGING"
...
!
 ...
 print*, " qg =", sum( qg)
 ! print*, " rain =", sum( rain)
 ! print*, " rainncv =", sum( rainncv)
 ! print*, " sr =", sum( sr)
   ! if(present(snowncv) .and. present(snow)) snowncv(i) = 0.
 if (present( snow)) print *, " snow = ", sum( snow)
 if (present( snowncv)) print *, " snowncv = ", sum( snowncv)
 if (present( graupel)) print *, " graupel = ", sum( graupel)
 if (present( graupelncv)) print *, " graupelncv = ", sum( graupelncv)
 if (present( rainprod2d)) print *, " rainprod2d = ", sum( rainprod2d)
 if (present( evapprod2d)) print *, " evapprod2d = ", sum( evapprod2d)



 ! call backtrace()
 ! stop "DEBUGGING BACKTRACE"
 
! stop "DEBUGGING in MP_WSM6.F90"
!
!----------------------------------------------------------------
...
enddo
 ...
 ! print *, "k", kts, kte
 ! print *, "i", its, ite
 ! print *, "shape(qr)", shape(qr)
 ! print *, "lbounds", lbound(qr)
 ! print *, "ubounds", ubound(qr)
...
!
   ...
   print *, "--- BAZ ---"
   print *, "fall = ", fall(its:ite,1,1:3)
   print *, "delqrs1 =", sum(delqrs1)
   print *, "delqrs2 =", sum(delqrs2)
   print *, "delqrs3 =", sum(delqrs3)
   print *, "/delz =", sum(delz)
   print *, "/dtcld =", dtcld
   do k = kts, kte
     do i = its, ite
...
!
   ! print *, "THIS SHOULD EQUAL STUFF ITS, ITE", its, ite
   do i = its, ite
     fallsum = fall(i,kts,1)+fall(i,kts,2)+fall(i,kts,3)+fallc(i,kts)
     print *, i, ": fallsum =", fallsum
...
!
 ...
 print *, "FOOBAR                 qmin = ", qmin
 print *, "DEBUGGG: SUM cloudice mr QI = ", sum(qi) ! = 0.0
 print *, "DEBUGGG: SUM rain mr     QR = ", sum(qr) ! = 0.0
 print *, "DEBUGGG: SUM snow mr     QS = ", sum(qs) ! = 0.0
 print *, "DEBUGGG: SUM graupel mr  QG = ", sum(qg) ! = 0.0
 print *, "DEBUGGG: SUM airtmep      t = ", sum(t)
 print *, "DEBUGGG: SUM wv mr        q = ", sum(q)
 print *, "DEBUGGG: SUM cl mr       qc = ", sum(qc) ! = 0.0
 print *, "DEBUGGG: SUM           rain = ", sum(rain)
 ! print *, "DEBUGGG: SUM        rainncv = ", sum(rainncv)
 print *, "DEBUGGG: SUM ratio sn/rn sr = ", sum(sr)
 ! print *, "DEBUGGG: SUM snow = ", sum()
 ! print *, "DEBUGGG: SUM  = ", sum()
 ! print *, "DEBUGGG: SUM  = ", sum()
 ! print *, "DEBUGGG: SUM  = ", sum()
 ! print *, "DEBUGGG: SUM  = ", sum()
 ! print *, "DEBUGGG: SUM  = ", sum()
...
!-----------------------------------------------------------------------------------------------------------------
 print *, "DEBUGGG ENTERED nislfv_rain_plm: im =", im
 precip(:) = 0.0
...
! skip for no precipitation for all layers
    allold = 0.0
    do k=1,km
       ! print *, i, ": qq(k)", qq(k) ! all 0, i=1,2
       allold = allold + qq(k)
    enddo
    if(allold.le.0.0) then
       cycle i_loop
    endif
    ! error stop "ST OP  RIGHT HERE"
!
...
!
    ! rain out
    ! DEBUGGG
    print *, "PRECIP K", 1, km, "precip bounds", lbound(precip), "|", ubound(precip)
...
!-----------------------------------------------------------------------------------------------------------------
 print *, "DEBUGGG ENTERED nislfv_rain_plm6: im =", im
 precip(:) = 0.0
 precip1(:) = 0.0
 precip2(:) = 0.0
...
! replace the new values
      ...
      print *, "nislfv_rain_plm6 precip", sum(precip1), sum(precip2)
    enddo ist_loop
